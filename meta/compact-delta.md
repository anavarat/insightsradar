...
- Carry-forward: Runtime config contract is strict: `INGESTION_MODE` in {`all`,`product_engineering_only`}, default start date is `2026-05-01T00:00:00Z`, and `PRIMARY_MODEL`/`FALLBACK_MODEL` must both be present and distinct.
...
- Carry-forward: Ingestion contract is hourly, canonical URL is unique identity, and eligibility gate applies date threshold plus ingestion mode filter before persistence.
...
- Carry-forward: Reprocessing trigger contract is hash-diff based; unchanged articles must not enqueue queue jobs.
...
- Carry-forward: Keyword contract uses hybrid signals with canonical-first normalization and top-N cap (default 5, hard max 8).
...
- Carry-forward: Digest generation policy is primary-model retries (3) then fallback-model retries (3), with terminal `digest_failed` after both paths exhaust.
...
- Carry-forward: Storage contract is D1 as query/status index and R2 as artifact store, with mandatory word-count metadata for article and all digest levels.
...
- Carry-forward: Architecture baseline is monorepo (`apps/worker`,`apps/web`,`packages/shared`) with native Worker handlers in v1 and Hono deferred.
...
- Carry-forward: Routing contract is `/api/*` + `/health` server-owned with SPA fallback for other GETs; feed ordering contract is cursor tuple `(published_at, canonical_url)`.
- Files: `docs/adr/0001-monorepo-structure-and-v1-deploy-shape.md`, `meta/compact-delta.md`.
...
- Carry-forward: Repo topology contract is workspace-based; worker runtime/tests/config live under `apps/worker` and web runtime under `apps/web`.
...
- Carry-forward: Tile feed API contract is cursor-based (`/api/articles`) with newest-first stable ordering and opaque cursor continuity.
...
- Carry-forward: View/API contract now includes summary/detail endpoints keyed by article ID and a required scrollable container for level2 digest rendering.
...
## 2026-05-10 01:37 AEST
- Change: Implemented slice #9 manual admin controls with token-protected `POST /api/admin/reprocess` and `POST /api/admin/backfill` endpoints, including backfill RSS eligibility scheduling and queue reasons for manual operations.
